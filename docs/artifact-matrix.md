...
4. **Companion PWA** — interactive field reference; field-ready offline hardening is a Phase 5 gate.
...
| Connectivity dependency | May use network in its existing product model; outside Companion Phase 0. | None. | None. | Phase 4 packages all field-critical resources locally and has no runtime external dependency, but does not claim offline cold-launch readiness until Phase 5 cache/update hardening and physical testing pass. |
...
Phase 4 implements the static shell, generated canonical dataset, friend first-open/install flow, standalone setup view, structural Offline Check, public-only sharing, device-local state, accessibility checks, and browser automation. Its development service worker deliberately performs no caching. The stable public URL is one build configuration value intended for a future Mountain Guide Crew tab; deployment remains deferred. Phase 5 owns production precaching, atomic activation, cold launch, mixed-version detection, update recovery, and physical Airplane Mode verification.
