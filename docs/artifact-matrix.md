...
| Medium | Existing website/product. | Exactly three 8.5 × 11-inch portrait pages, intended for a waterproof sleeve or waterproof print after physical approval. | About 3.5 × 5 inches, laminatable, one page or two-sided. | Installed offline-first PWA on primary and backup iPhones. |
...
| Provenance display | Existing Mountain Guide version/release. | Every page visibly shows product, data version, source release, generated date, verified date, short manifest fingerprint, draft status, and page number; the artifact record and PDF metadata carry the full hash and source commit. | Compact identity with data version, source pin, manifest SHA-256, and verified_at on each side. | Persistent/offline identity surface recording data version, source pin, manifest SHA-256, and verified_at. |
