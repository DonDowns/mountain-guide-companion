...
Candidate.8 local-state schema version 4 fails closed to empty defaults on corrupt or incompatible input. Migration from schema versions 1 through 3 preserves allowlisted operational/private values, adds only an empty onboarding record, does not invent timestamps for earlier boolean-only milestone marks, and does not reinterpret the earlier structural check as a verified offline bundle. Offline verification is bound to the exact bundle ID, and the physical Airplane Mode mark is a local user attestation only. Public Companion sharing sends only the configured public Companion URL. Diagnostics and copy-only reports exclude private contacts, notes, actual starts, milestones, and device identifiers. Prepared milestone messages contain only approved template wording, the selected canonical objective name where applicable, and the current America/Denver time. Copy or Share never changes the local milestone mark and never claims delivery.
